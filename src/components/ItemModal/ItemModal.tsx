...
export const ItemModal = (props: ItemModalPropsType) => {
    ...
    return (<div className="fixed z-50 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    <div  className="fixed inset-0 bg-zappyblack bg-opacity-75 transition-opacity" aria-hidden="true"></div>
                    ...
                    <div className="inline-block px-4 align-bottom bg-white rounded-2xl text-left overflow-hidden transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        <div className="bg-white relative pt-5 pb-4 sm:pb-4">
                            <Link to={'/'} className="absolute top-1 right-0 font-medium text-lg text-zappyblack">X</Link>
                            <h2 className="text-zappyblack text-center text-lg font-bold mb-4 leading-6">{item.title}</h2> 
                            <figure className="relative overflow-hidden">
                                {item.available ? <div className="absolute bg-zappybadge z-10 text-xs font-medium top-4 left-4 text-zappyblack px-4 py-2 border-transparent rounded-2xl">Disponibile subito</div> : null}
                                <img className="object-cover h-40 w-full" src={item.images[0].url} alt={item.street} />
                            </figure> 
                            <div className="card-body py-6">
                                <div className="flex items-center justify-center mx-auto">
                                    <div className="pr-6"><span className="text-zappyblack">{item.tenants}</span> <span className="text-xs">inquilini</span></div>
                                    <div className="pr-6"><span className="text-zappyblack">{item.baths}</span> <span className="text-xs">bagno</span></div>
                                    <div><span className="text-zappyblack">{item.beds}</span> <span className="text-xs">letto</span></div>
                                </div>
                                ...
                                <div className="flex items-center justify-center text-lg">
                                    ...
                                    <p><span className="text-zappyblack font-bold">€ {item.price}</span> /mese</p>
                                </div>
